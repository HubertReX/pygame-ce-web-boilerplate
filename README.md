...
- postprocessing OpenGL/WebGL **shaders** (`zengl`[^5]): _saturated_, _black&white_, _retro CRT monitor_
...
- **NPCs** follow individual lists of waypoints defined inside `Tiled`[^2] map or automatically moving towards set target (e.g. **Player**)
...
- test **pixel editors**:
  ...
  - [aseprite] - `$20` or build own for [MacOs] or [Win] `free` copy
...
- try out the [Thorpy] for **UI/HUD** - as full replacement of current **menus** or at least the **heterogeneous texts** for mixed text and formatting (see [heterogeneous_texts]) - can be useful in **dialogs**
