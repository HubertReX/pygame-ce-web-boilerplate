...
from settings import PARTICLES_DIR, WIDTH, HEIGHT
    ...
    def emit(self, dt: float):
        ...
        if self.particles:
            ...
            for particle in self.particles:
                ...
                particle.x += (particle.speed_x * MILI_SEC * dt) + \
                    (math.sin(particle.time_elapsed * MILI_SEC * 10) * 4)
                ...
                self.screen.blit(surface, (particle.x - int(surface.get_width() // 2),
                                 particle.y - int(surface.get_height() // 2)))
...
class ParticleLeafs():
    ...
    def add(self, **kwargs):
        ...
        self.particle.add_particles(start_pos=pygame.mouse.get_pos(), move_speed=80,
                                    move_dir=210 + random.randint(-30, 30), scale=5, lifetime=4)
...
class ParticleRain():
    ...
    def add(self, **kwargs):
        # move 80 pixels/seconds into south-west (down-left) +/- 30 degree, enlarge 5 x, kill after 4 seconds
        self.particle.add_particles(start_pos=pygame.mouse.get_pos(), move_speed=1500,
                                    move_dir=210 + random.randint(-30, 30), scale=5, lifetime=0.5)
