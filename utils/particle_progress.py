...
def main():
    ...
    spawn_rect = pygame.Rect(0,0,800, 50)
    leaf_img = pygame.image.load('Leaf_single.png').convert_alpha()
    particle_leaf = ParticleImageBased(screen=screen, img=leaf_img, rate=0.5, scale_speed=0.125, alpha_speed=0.125, rotation_speed=0.0, spawn_rect=spawn_rect)
    PARTICLE_EVENT = particle_leaf.custom_event_id
    ...
    while True:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                particle_leaf.add_particles(start_pos=pygame.mouse.get_pos(), move_speed=40, move_dir=230 + random.randint(-30, 30), scale=5, lifetime=8)

        dt = clock.tick(30)
        screen.fill((30,30,30))
        particle_leaf.emit(dt)
