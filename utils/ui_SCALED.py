...
class UI:
    def __init__(self, scene: Scene) -> None:
        ...
        border = 6 * INVENTORY_ITEM_SCALE
        self.modal_panel_bg = NinePatch(
            file="nine_patch_03c.png",
            scale=INVENTORY_ITEM_SCALE).get_scaled_to(WIDTH - 100 * SCALE, HEIGHT - 50 * SCALE)
        self.dialog_panel_bg = NinePatch(file="nine_patch_01c.png",
                                         scale=INVENTORY_ITEM_SCALE).get_scaled_to(WIDTH - 100 * SCALE, HEIGHT // 3)
        self.name_panel_bg = NinePatch(file="nine_patch_13.png", scale=INVENTORY_ITEM_SCALE).get_scaled_to(
            13 * TILE_SIZE * SCALE, 0.5 * TILE_SIZE * SCALE)
        self.available_action_bg = NinePatch(
            file="panel_brown.png", scale=INVENTORY_ITEM_SCALE, border=3).get_scaled_to(100 * SCALE, 18 * SCALE)
        # self.notification_bg = NinePatch(file="panel_brown.png", scale=INVENTORY_ITEM_SCALE, border=3)
        self.notification_bg = NinePatch(file="nine_patch_04c.png", scale=INVENTORY_ITEM_SCALE, border=3)

        weapon_s = (12 + TILE_SIZE) * SCALE * 2
        self.weapon_bg = NinePatch(file="nine_patch_04.png",
                                   scale=INVENTORY_ITEM_SCALE).get_scaled_to(weapon_s, weapon_s)
        self.stats_bg = NinePatch(file="nine_patch_04.png",
                                  scale=INVENTORY_ITEM_SCALE).get_scaled_to(150 * SCALE, 80 * SCALE)

        help_w = int(6 * FONT_SIZE_MEDIUM * 1.1) * SCALE
        self.help_bg = NinePatch(file="nine_patch_04.png",
                                 scale=INVENTORY_ITEM_SCALE).get_scaled_to(200 * SCALE, help_w)
        ...
        if self.show_nine_patch_test:
            self.np_01 = NinePatch(file="nine_patch_01.png", scale=INVENTORY_ITEM_SCALE).get_scaled_to_image(text_surf)
            self.np_02 = NinePatch(file="nine_patch_02.png", scale=INVENTORY_ITEM_SCALE).get_scaled_to_image(text_surf)
            self.np_03 = NinePatch(file="nine_patch_03.png", scale=INVENTORY_ITEM_SCALE).get_scaled_to_image(text_surf)
            self.np_04 = NinePatch(file="nine_patch_04.png", scale=INVENTORY_ITEM_SCALE).get_scaled_to_image(text_surf)
            self.np_05 = NinePatch(file="nine_patch_05.png", scale=INVENTORY_ITEM_SCALE).get_scaled_to_image(text_surf)
            self.np_06 = NinePatch(file="nine_patch_06.png", scale=INVENTORY_ITEM_SCALE).get_scaled_to_image(text_surf)
            self.np_07 = NinePatch(file="nine_patch_07.png", scale=INVENTORY_ITEM_SCALE).get_scaled_to_image(text_surf)
            self.np_08 = NinePatch(file="nine_patch_08.png", scale=INVENTORY_ITEM_SCALE).get_scaled_to_image(text_surf)
            self.np_09 = NinePatch(file="nine_patch_09.png", scale=INVENTORY_ITEM_SCALE).get_scaled_to_image(text_surf)
            self.np_10 = NinePatch(file="nine_patch_10.png", scale=INVENTORY_ITEM_SCALE).get_scaled_to_image(text_surf)
            self.np_11 = NinePatch(file="nine_patch_11.png", scale=INVENTORY_ITEM_SCALE).get_scaled_to_image(text_surf)
            self.np_12 = NinePatch(file="nine_patch_12.png", scale=INVENTORY_ITEM_SCALE).get_scaled_to_image(text_surf)
        ...
        self.modal_panel_offset = (50 * SCALE, 25 * SCALE)
        ...
        self.dialog_panel_offset = (50 * SCALE, HEIGHT - self.dialog_panel_bg.get_rect().height - 10)
    ...
    def selection_box(self, left: int, top: int, has_switched: bool, cooldown: int = 100) -> pygame.Rect:
        ...
        # nine patch panel
        surface.blit(self.weapon_bg, bg_rect.move(-6 * SCALE, -6 * SCALE).topleft)
    ...
    def show_weapon_panel(self, weapon: ItemSprite | None, has_switched: bool, elapsed_time: float) -> None:
        ...
        bg_rect = self.selection_box(TILE_SIZE, HEIGHT - (TILE_SIZE * 4 * SCALE), has_switched, cooldown)
        if weapon:
            weapon_surf = pygame.transform.scale(
                weapon.image_directions["up"], (TILE_SIZE * 3 * SCALE, TILE_SIZE * 3 * SCALE))
            ...
            if cooldown == 100:
                pos = bg_rect.move(2 * SCALE, 9 * SCALE).topright
        ...
        if self.show_help_info:
            ...
            for i, action in enumerate(show_actions, start=1):
                self.game.render_text(
                    ...
                    f"{action['msg']}",
                    (WIDTH - (200 + 16) * SCALE, (20 + int(i * FONT_SIZE_MEDIUM * 1.1))) * SCALE,
                    shadow = True
                )
                self.display_surface.blit(
                    self.icons_dict[action['show'][0]][0],
                    (WIDTH - 200 * SCALE, (16 + int(i * FONT_SIZE_MEDIUM * 1.1)) * SCALE))
        # else:
    ...
    def show_action(self, action: str, row: int, label: str = "") -> None:
        ...
        self.display_surface.blit(self.available_action_bg,
                                  (WIDTH - (TILE_SIZE + 100) * SCALE,
                                   HEIGHT - (2 * TILE_SIZE + row_spacing) * SCALE))
        self.display_surface.blit(icon,
                                  (WIDTH - (TILE_SIZE + 8) * SCALE,
                                   HEIGHT - (TILE_SIZE - 14 - row_spacing) * SCALE))
        self.display_text(label, (WIDTH - (TILE_SIZE + label_w - 10) * SCALE,
                                  HEIGHT - (TILE_SIZE + 7 + row_spacing) * SCALE), border=True)
    ...
    def show_notification(self, notification: Notification, row: int) -> None:
        ...
        NOTIFICATION_Y_TOP: int = 115 * SCALE
        ...
        # scale = 2
        rich_text = self.create_rich_text(
            notification.message, (notification.width + 45 * SCALE, notification.height + 30 * SCALE))
        rich_text.on_update(self.game.time_elapsed)

        self.display_surface.blit(
            self.notification_bg.get_scaled_fit(notification.width + 18 * SCALE, notification.height + 2 * SCALE),
            (TILE_SIZE, y))
    ...
    def display_ui(self, elapsed_time: float) -> None:
        ...
        # upper right corner
        # FPS counter
        self.box(WIDTH - 100, TILE_SIZE, 90, 18)
        self.display_text(f"FPS: {self.game.fps:5.1f}", (WIDTH - 100, TILE_SIZE + 10))
    ...
    def show_stats_panel(self, player: Player) -> None:
        self.display_surface.blit(self.stats_bg, (TILE_SIZE // 2, TILE_SIZE // 2))
